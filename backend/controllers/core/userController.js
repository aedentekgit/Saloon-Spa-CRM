...
const LOGIN_LOCK_MS = Number(process.env.LOGIN_LOCK_MS) || 60 * 60 * 1000;
...
exports.loginUser = async (req, res) => {
  try {
    ...
    const { isActive: roleIsActive, permissions } = await resolveRoleAccess(effectiveRole, source);
...
exports.getUserProfile = async (req, res) => {
  try {
    ...
    if (user) {
      const effectiveRole = normalizeAuthRole(user.role, source);
      const { permissions } = await resolveRoleAccess(effectiveRole, source);
      res.json({
        _id: user._id,
        name: user.name,
        email: user.email,
