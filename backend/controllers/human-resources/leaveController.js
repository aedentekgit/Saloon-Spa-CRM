...
const getLeaves = async (req, res) => {
  try {
    ...
    } else if (req.user.role === 'Employee' || req.user.role === 'Client' || (req.authSource === 'Employee' && req.user.role !== 'Manager' && req.user.role !== 'Admin')) {
...
const createLeave = async (req, res) => {
  ...
  try {
    ...
    const canCreateForStaff = req.user.role === 'Admin' || req.user.role === 'Manager';
...
const updateLeaveStatus = async (req, res) => {
  try {
    ...
    const isBranchManager = req.user.role === 'Manager' && isBranchMatch;
