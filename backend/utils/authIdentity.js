...
const normalizeAuthRole = (roleName, accountSource) => {
  ...
  // Preserve custom roles so they can be looked up in the database.
  if (role) return role;
...
const resolveRoleAccess = async (roleName, accountSource) => {
  const role = String(roleName || '').trim();
  const roleData = role ? await Role.findOne({ name: role }).lean() : null;

  if (!roleData) {
    // If custom role is not found in DB, fallback to defaults based on account source
    let fallbackRole = role;
    if (!DEFAULT_ROLE_PERMISSIONS[role]) {
      if (accountSource === ACCOUNT_SOURCES.EMPLOYEE) {
        fallbackRole = 'Employee';
      } else if (accountSource === ACCOUNT_SOURCES.CLIENT) {
        fallbackRole = 'Client';
      } else {
        fallbackRole = 'Client';
      }
