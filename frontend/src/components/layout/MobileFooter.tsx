...
const MobileFooter: React.FC = () => {
  ...
  const sheetItems = [
    ...
    { name: 'Reports', icon: BarChart3, path: '/reports', permission: ['reports', 'finance'] },
    ...
    { name: 'Admin Users', icon: UserRound, path: '/admins', permission: ['admins', 'roles'] },
  ...
  return (
    <>
      ...
      <div className="lg:hidden fixed bottom-[calc(0.5rem+env(safe-area-inset-bottom,0px))] sm:bottom-[calc(0.75rem+env(safe-area-inset-bottom,0px))] left-2 sm:left-4 right-2 sm:right-4 z-[100] animate-in slide-in-from-bottom-10 duration-700">
        <nav className="bg-white rounded-2xl sm:rounded-[1rem] border border-zen-stone shadow-2xl shadow-zen-brown/15 p-1 sm:p-1.5 grid grid-cols-5 gap-1 relative ring-1 ring-black/5">
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex min-w-0 flex-col items-center justify-center px-1 py-2 rounded-xl transition-all duration-300 gap-1 ${
                  isActive
                    ? 'text-zen-sand bg-zen-sand/10'
                    : 'text-zen-brown/65 hover:text-zen-brown active:scale-90'
                }`
              }
            >
              {({ isActive }) => (
          ...
          <button
            ...
            className={`flex min-w-0 flex-col items-center justify-center px-1 py-2 rounded-xl transition-all duration-300 gap-1 ${
              isMoreOpen ? 'text-zen-sand bg-zen-sand/10' : 'text-zen-brown/65 hover:text-zen-brown active:scale-95'
            }`}
          >
        ...
        {isMoreOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-0 left-0 right-0 bg-white z-[120] lg:hidden rounded-t-[2rem] sm:rounded-t-[3rem] shadow-2xl max-h-[85vh] sm:max-h-[90vh] overflow-hidden flex flex-col border-t border-zen-stone"
            >
              <div className="p-4 sm:p-5 border-b border-zen-stone flex items-center justify-between shrink-0 bg-white">
                <div>
                  <h3 className="text-lg sm:text-xl font-serif font-bold text-zen-brown tracking-tight">All Sections</h3>
                  <p className="text-[9px] sm:text-[10px] font-bold text-zen-brown/60 uppercase tracking-[0.24em]">Explore all sectors</p>
                </div>
                <button
                  onClick={() => setIsMoreOpen(false)}
                  className="w-10 h-10 rounded-xl bg-white border border-zen-stone flex items-center justify-center text-zen-brown/65 hover:text-zen-brown transition-all duration-500 shadow-sm"
                >
                  <X size={20} />
                </button>
              </div>
                 ...
                 <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                  {filteredSheet.map((item) => (
                    <NavLink
                      ...
                      to={item.path}
                      onClick={() => setIsMoreOpen(false)}
                      className="flex min-h-[104px] flex-col items-center justify-center gap-1.5 p-3 py-4 rounded-2xl sm:rounded-3xl bg-white hover:bg-zen-cream hover:shadow-lg transition-all duration-500 border border-zen-stone group text-center"
                    >
                      <div className="w-11 h-11 rounded-[1.25rem] bg-white flex items-center justify-center text-zen-brown/65 group-hover:text-zen-sand transition-all duration-500 group-hover:bg-zen-sand/10 shrink-0 border border-zen-stone">
              ...
              <div className="p-5 shrink-0 text-center text-[9px] font-serif italic text-zen-brown/55 bg-white border-t border-zen-stone">
