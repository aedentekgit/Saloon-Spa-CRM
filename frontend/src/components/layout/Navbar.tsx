...
const Navbar = ({
  ...
  return (
    <header className="h-14 sm:h-16 md:h-[72px] bg-white border-b border-zen-stone flex items-center justify-between px-3 sm:px-4 md:px-6 lg:px-10 sticky top-0 z-40 shadow-sm overflow-visible">
      ...
      <div className="flex items-center gap-2 sm:gap-4 min-w-0">
        ...
        {user?.role !== 'Client' && (
          <>
            <button
              onClick={onMenuClick}
              className="lg:hidden text-zen-brown/65 hover:text-zen-brown transition-colors p-1.5 rounded-lg hover:bg-zen-cream"
            ...
            <button
              ...
              className="hidden lg:flex text-zen-brown/65 hover:text-zen-brown transition-colors p-1.5 rounded-lg hover:bg-zen-cream"
        ...
        {user?.role === 'Client' ? (
          <nav className="hidden sm:flex items-center gap-1 bg-white rounded-xl p-1 border border-zen-stone shadow-sm">
            {[
                ...
                className={({ isActive }) =>
                  `flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-black tracking-wide transition-all duration-200 ${
                    isActive
                      ...
                      : 'text-zen-brown/70 hover:text-zen-brown hover:bg-zen-cream'
                  }`
                }
              >
                <Icon size={13} />
      ...
      <div className="flex items-center gap-1.5 sm:gap-4">
        ...
        <div className="relative" ref={notifRef}>
           <button
              ...
              className={`relative p-2 transition-all rounded-lg ${isNotifOpen ? 'bg-zen-cream text-zen-sand' : 'text-zen-brown/65 hover:text-zen-brown hover:bg-zen-cream'}`}
           >
              <Bell size={18} />
              {unreadCount > 0 && (
             ...
             {isNotifOpen && (
               ...
               >
                 <div className="px-4 py-3 border-b border-zen-stone/30 flex items-center justify-between bg-white">
                     <div>
                        <h4 className="text-[10px] font-black uppercase tracking-[0.22em] text-zen-brown/70">Notifications</h4>
                        <p className="text-[10px] font-bold text-zen-brown/55 mt-0.5">{unreadCount} unread</p>
                     </div>
                  ...
                  <div className="max-h-[min(24rem,calc(100vh-9rem))] overflow-y-auto py-1">
                     {notifications.length === 0 ? (
                        <div className="py-12 px-6 text-center">
                           <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center mx-auto mb-4 text-zen-brown/55 border border-zen-stone">
                              <Bell size={20} strokeWidth={1} />
                           </div>
                           <p className="text-[11px] font-black uppercase tracking-widest text-zen-brown/60">Your sanctuary is quiet</p>
                           <p className="text-[10px] text-zen-brown/50 mt-1">No new updates right now.</p>
                        </div>
                     ) : (
                        notifications.map((notif) => (
                           <div
                              ...
                              className={`px-4 py-3 hover:bg-zen-cream transition-colors cursor-pointer group border-b border-zen-stone/40 last:border-b-0 ${!notif.isRead ? 'bg-zen-sand/[0.06]' : ''}`}
                           >
                              <div className="flex items-start gap-3 min-w-0">
                                 ...
                                 <div className="flex-1 min-w-0">
                                    <div className="flex items-start justify-between gap-3">
                                       ...
                                       <span className="text-[9px] font-bold text-zen-brown/55 uppercase shrink-0 pt-0.5">{new Date(notif.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                    </div>
                  ...
                  <div className="px-4 py-3 border-t border-zen-stone/30 text-center bg-white">
                     <button className="text-[10px] font-black uppercase tracking-[0.18em] text-zen-brown/60 hover:text-zen-brown transition-colors">View all updates</button>
                  </div>
               </motion.div>
             )}
           </AnimatePresence>
        ...
        <div className="relative" ref={dropdownRef}>
          <div
            className="flex items-center gap-3 cursor-pointer group hover:bg-zen-cream p-1 rounded-2xl transition-all pr-4 border border-transparent hover:border-zen-stone"
          ...
          >
            <div className="w-10 h-10 professional-frame classic-shine-effect flex items-center justify-center bg-white shadow-sm border-zen-stone/50 shrink-0">
              <div className="w-full h-full bg-white flex items-center justify-center text-zen-brown/65 group-hover:text-zen-brown transition-colors">
                <UserRound size={18} />
              </div>
            </div>
            <div className="hidden md:flex flex-col">
              ...
              <span className="text-[9px] uppercase tracking-[0.2em] font-black text-zen-brown/55 mt-0.5 min-h-[0.75rem] flex items-center">
                {validating ? (
