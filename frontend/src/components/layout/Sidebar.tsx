...
const Sidebar = ({
  ...
  const menuGroups = [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'Reports', icon: TrendingUp, path: '/reports', permission: ['reports', 'finance'] },
      ]
    },
    {
      label: 'Comms',
      ...
      className={({ isActive }) =>
        `flex items-center rounded-xl transition-all duration-300 group min-h-[36px] ${
          isCollapsed && !isMobile
            ...
            : `${isChild ? 'pl-3 pr-2.5 py-1.5' : 'px-3 py-2'} hover:bg-zen-cream`
        } ${isActive ? 'text-white shadow-sm shadow-zen-sand/20' : 'text-zen-brown/70 hover:text-zen-brown'}`
      }
    >
      {({ isActive }) => (
        <>
          ...
          {(!isCollapsed || isMobile) && (
            <span className={`ml-2.5 text-[12px] font-bold tracking-wide flex-1 truncate ${isActive ? 'text-white' : 'text-zen-brown/75 group-hover:text-zen-brown'}`}>
              {item.name}
            </span>
          )}
        </>
      ...
      <div key={item.name} className="space-y-1">
        <button
          ...
          className={`w-full flex items-center rounded-xl transition-all duration-300 group min-h-[36px] ${
            isCollapsed && !isMobile ? 'justify-center p-2.5 mx-auto w-10 h-10' : 'px-3 py-2 hover:bg-zen-cream'
          } ${
            isChildActive && !isOpen
              ? 'text-zen-sand bg-zen-cream font-bold'
              : 'text-zen-brown/70 hover:text-zen-brown'
          }`}
        >
          <div className="flex items-center justify-center shrink-0">
          ...
          {(!isCollapsed || isMobile) && (
            <>
              <span className={`ml-2.5 text-[12px] font-semibold tracking-wide flex-1 text-left truncate ${
                isChildActive ? 'text-zen-sand font-bold' : 'text-zen-brown/75 group-hover:text-zen-brown'
              }`}>
                {item.name}
              </span>
              <ChevronDown
                size={14}
                className={`transform transition-transform duration-300 text-zen-brown/55 group-hover:text-zen-brown ${
                  isOpen ? 'rotate-180' : ''
                }`}
              />
            </>
  ...
  return (
    <aside className={`bg-white border-r border-zen-stone h-full transition-all duration-300 ease-in-out flex flex-col z-50 rounded-none relative overflow-hidden shadow-sm ${isCollapsed ? 'lg:w-[68px] w-[min(90vw,16rem)] sm:w-[min(84vw,16rem)]' : 'lg:w-[210px] w-[min(92vw,18rem)] sm:w-[min(88vw,17rem)]'}`}>

      {/* Top Logo Section */}
      <div
        className={`flex items-center px-4 sm:px-6 relative overflow-hidden transition-all duration-500 ${isCollapsed && !isMobile ? 'h-14 justify-center' : 'h-16 sm:h-20 justify-center'}`}
      >
        ...
        <div className={`relative z-10 flex items-center justify-center transition-all duration-700 ${isCollapsed && !isMobile ? 'scale-90' : 'scale-100'}`}>
          {logoUrl ? (
            <div className={`
              relative p-1.5 bg-white classic-shine-effect
              ...
              border-zen-stone transition-all duration-500
              shadow-none
            ...
            <div className={`
              ...
              professional-frame classic-shine-effect bg-white border border-zen-stone flex items-center justify-center
              shadow-none
            `}>
        ...
        {isMobile && onClose && (
          <button
            onClick={onClose}
            className="relative z-20 w-10 h-10 rounded-full bg-white border border-zen-stone flex items-center justify-center text-zen-brown shadow-sm active:scale-95 transition-all"
          >
            <X size={20} />
          </button>
        )}
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-2 scrollbar-hide">
        {filteredGroups.map((group, gi) => (
          <div key={group.label} className={gi > 0 ? 'mt-2' : ''}>
            ...
            {(!isCollapsed || isMobile) && (
              <p className="px-3 mb-1 text-[9px] font-black uppercase tracking-[0.26em] text-zen-brown/55 select-none">
                {group.label}
              </p>
            )}
      ...
      <div className="p-3 relative group overflow-hidden mt-auto" onClick={() => setShowLogoutConfirm(true)}>
        ...
        <div className="relative z-10 flex items-center justify-between p-1.5 rounded-xl hover:bg-zen-cream transition-all duration-500 cursor-pointer border border-transparent hover:border-zen-stone">
          <div className="flex items-center gap-2.5">
            ...
            {(!isCollapsed || isMobile) && (
              <div className="flex flex-col min-w-0">
                ...
                <span className="text-[9px] font-black uppercase tracking-[0.15em] text-zen-brown/55 group-hover:text-red-500 transition-colors">Session Profile</span>
              </div>
            )}
          </div>
          {(!isCollapsed || isMobile) && (
            <LogOut size={14} className="text-zen-brown/55 group-hover:text-red-500 transition-all shrink-0 hover:scale-110" />
          )}
        </div>
      </div>
