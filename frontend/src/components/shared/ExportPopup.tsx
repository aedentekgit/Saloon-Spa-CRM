...
export const ExportPopup = <T extends object>({
  ...
  const hasSerialColumn = columns.some((col) => {
    const header = col.header.trim().toLowerCase().replace(/\s+/g, ' ');
    return header === 's no' || header === 's.no' || header === 's.no.' || header === 'serial no' || header === 'serial number';
  });
  const hasDateColumn = columns.some((col) => col.header.trim().toLowerCase() === 'date');
  const exportHeaders = [
    ...(!hasSerialColumn ? ['S No'] : []),
    ...(!hasDateColumn ? ['Date'] : []),
    ...columns.map((col) => col.header)
  ];

  const getRecordDate = (item: T) => {
    const record = item as Record<string, unknown>;
    const candidate = record.date || record.createdAt || record.updatedAt;
    if (!candidate) return exportDate;
    const parsed = new Date(String(candidate));
    return Number.isNaN(parsed.getTime()) ? String(candidate) : parsed.toISOString().slice(0, 10);
  };

  const rows = useMemo(
    () =>
      data.map((item, index) => [
        ...(!hasSerialColumn ? [(index + 1).toString()] : []),
        ...(!hasDateColumn ? [getRecordDate(item)] : []),
        ...columns.map((col) => {
          const rawValue =
    ...
    [data, columns, hasDateColumn, hasSerialColumn, exportDate]
  );
  ...
  const buildCsv = (exportRows: string[][]) => {
    const headerLine = exportHeaders.map((header) => escapeCsv(header)).join(',');
