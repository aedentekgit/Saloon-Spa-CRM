          ...
          <div className="zen-pointed-surface border border-zen-stone bg-white shadow-none px-5 sm:px-6 py-4 mb-4">
            <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-4 sm:gap-5 lg:gap-8 min-w-0">
                ...
                <div className={`w-full ${searchMaxWidth} flex items-center gap-4 min-w-0`}>
                  <div className="relative group flex-1">
                    ...
                    <input
                      ...
                      className="w-full h-[52px] pl-[52px] pr-6 bg-white border border-zen-stone rounded-[1.15rem] focus:bg-white focus:ring-4 focus:ring-zen-sand/5 focus:border-zen-sand/20 outline-none transition-all duration-500 text-sm font-medium shadow-sm"
                    />
                  </div>
                </div>
              )}
              ...
              {!hideViewToggle && onViewModeChange && (
                <div className="flex items-center h-[48px] sm:h-[52px] bg-white p-1 rounded-[1.15rem] border border-zen-stone shadow-inner shrink-0 relative">
                    <button
                      onClick={() => onViewModeChange('grid')}
                      className={`h-full aspect-square px-4 flex items-center justify-center rounded-xl transition-all duration-500 relative z-10 ${viewMode === 'grid' ? 'bg-zen-brown text-white shadow-md' : 'text-zen-brown/30 hover:text-zen-brown hover:bg-zen-cream'}`}
                    >
                      <Grid size={18} />
                    </button>
                    <button
                      onClick={() => onViewModeChange('table')}
                      className={`h-full aspect-square px-4 flex items-center justify-center rounded-xl transition-all duration-500 relative z-10 ${viewMode === 'table' ? 'bg-zen-brown text-white shadow-md' : 'text-zen-brown/30 hover:text-zen-brown hover:bg-zen-cream'}`}
                    >
                      <List size={18} />
                    </button>
                  </div>
