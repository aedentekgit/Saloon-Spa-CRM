      ...
      <div className="relative z-10 flex flex-col gap-6">
        <div className="flex items-start justify-between gap-4">
          <div className="flex flex-col gap-2 min-w-0 flex-1">
            <h5 className="text-[10px] sm:text-[11px] font-black text-zen-brown/25 uppercase tracking-[0.3em] leading-tight">
              {label}
            </h5>
            <p className="text-3xl sm:text-4xl font-serif font-black text-zen-brown tracking-tighter leading-none truncate group-hover:translate-x-1 transition-transform duration-700 origin-left">
              {value}
            </p>
          </div>
        ...
        <div className="flex items-center justify-between pt-2">
          {trend ? (
            <div className="flex items-center gap-2.5 px-4 py-2 rounded-xl bg-zen-cream/40 border border-zen-stone/60 backdrop-blur-sm group-hover:bg-white transition-all duration-500">
               <Clock size={12} className="text-zen-sand/60" />
               <span className="text-[10px] text-zen-brown/50 font-black uppercase tracking-widest">{trend}</span>
            </div>
          ) : (
