...
const Reports = () => {
  ...
  const { invoices, expenses, services, appointments, employees, loading } = useData();
  ...
  const employeeReportData = useMemo(() => {
    const specialistMetrics = new Map<string, { totalServices: number; completedServices: number; revenue: number }>();

    (employees || []).forEach(emp => {
      specialistMetrics.set(emp.name, { totalServices: 0, completedServices: 0, revenue: 0 });
    });

    filteredAppointments.forEach(apt => {
      const name = apt.employee || (typeof apt.employeeId === 'object' && apt.employeeId?.name) || '';
      if (!name) return;
      if (!specialistMetrics.has(name)) {
        specialistMetrics.set(name, { totalServices: 0, completedServices: 0, revenue: 0 });
      }
      const metrics = specialistMetrics.get(name)!;
    ...
    filteredInvoices.forEach(inv => {
      (inv.items || []).forEach(item => {
        const name = item.specialistName || item.specialist || '';
        if (!name) return;
        if (!specialistMetrics.has(name)) {
          specialistMetrics.set(name, { totalServices: 0, completedServices: 0, revenue: 0 });
        }
        const metrics = specialistMetrics.get(name)!;
        metrics.revenue += (Number(item.price) || 0) * (Number(item.quantity) || 1);
      });
    });

    return Array.from(specialistMetrics.entries()).map(([name, metrics]) => {
  ...
  }, [employees, filteredAppointments, filteredInvoices]);

  const employeeReportColumns = useMemo<ExportColumn<any>[]>(
    () => [
      { header: 'Specialist Name', accessor: (row) => row.name },
      { header: 'Role', accessor: (row) => row.role },
      { header: 'Total Services Booked', accessor: (row) => row.totalServices },
      { header: 'Completed Services', accessor: (row) => row.completedServices },
      { header: 'Revenue Generated', accessor: (row) => `${settings?.general?.currencySymbol || 'QR'} ${row.revenue.toLocaleString()}` }
    ],
    [settings?.general?.currencySymbol, employees]
  );
  ...
  const financialReportData = useMemo(() => {
    ...
    const start = dayjs(dateWindow.startDate);
    const end = dayjs(dateWindow.endDate);
        ...
        <div className="flex gap-4">
           <ZenButton variant="secondary" className="px-6 py-4 flex items-center gap-3">
             ...
             <ExportPopup<any>
                ...
                fileName="specialist_performance_report"
                title="Specialist Impact Report"
                triggerLabel="Export Report"
                description="Download a detailed report of specialist bookings, completions, and revenue generated in the selected range."
             />
        </div>
      </div>
