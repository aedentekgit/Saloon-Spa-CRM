...
import {
  ...
  AlertTriangle, Slash, CheckCircle2, Clock, BarChart3, Users, Phone, Mail
} from 'lucide-react';
...
import { ZenAutocomplete, ZenDatePicker, ZenDropdown, ZenInput, ZenTextarea, useFloatingAnchor } from '../../components/zen/ZenInputs';
...
const Appointments = () => {
  ...
  const clientSearchOptions = useMemo(() => {
     return (rawClients || [])
       .map(c => ({
         id: c.name,
         name: c.name,
         email: [c.clientId, c.phone, c.email].filter(Boolean).join(' · ') || 'Registry Member'
       }));
  }, [rawClients]);

  const normalizeClientName = (name = '') => String(name || '').trim().toLowerCase();
  const normalizeClientEmail = (email = '') => String(email || '').trim().toLowerCase();
  const normalizeClientPhone = (phone = '') => String(phone || '').replace(/\D/g, '');
  const isSameClientName = (left = '', right = '') => {
    const normalizedLeft = normalizeClientName(left);
    const normalizedRight = normalizeClientName(right);
    return Boolean(normalizedLeft && normalizedRight && normalizedLeft === normalizedRight);
  };

  const isSameClientEmail = (left = '', right = '') => {
    const normalizedLeft = normalizeClientEmail(left);
    const normalizedRight = normalizeClientEmail(right);
    return Boolean(normalizedLeft && normalizedRight && normalizedLeft === normalizedRight);
  };

  const isSameClientPhone = (left = '', right = '') => {
    const normalizedLeft = normalizeClientPhone(left);
    const normalizedRight = normalizeClientPhone(right);
    return Boolean(normalizedLeft && normalizedRight && normalizedLeft === normalizedRight);
  };

  const referralCustomerOptions = useMemo(() => {
    const selectedClientName = normalizeClientName(formData.client);
    const selectedClientEmail = normalizeClientEmail(formData.clientEmail);
    const selectedClientPhone = normalizeClientPhone(formData.clientPhone);
    return clientSearchOptions.filter((option: any) => {
      const client = rawClients.find((c: any) => normalizeClientName(c.name) === normalizeClientName(option.name));
      return (
        normalizeClientName(option.name) !== selectedClientName &&
        !isSameClientEmail(client?.email, selectedClientEmail) &&
        !isSameClientPhone(client?.phone, selectedClientPhone)
      );
    });
  }, [clientSearchOptions, formData.client, formData.clientEmail, formData.clientPhone, rawClients]);

  const getClientByName = (name = '') => rawClients.find((client: any) =>
    normalizeClientName(client.name) === normalizeClientName(name)
  );

  const getClientByIdentity = (value = '') => {
    const normalizedName = normalizeClientName(value);
    const normalizedEmail = normalizeClientEmail(value);
    const normalizedPhone = normalizeClientPhone(value);
    return rawClients.find((client: any) => (
      normalizeClientName(client.name) === normalizedName ||
      normalizeClientEmail(client.email) === normalizedEmail ||
      normalizeClientPhone(client.phone) === normalizedPhone ||
      normalizeClientName(client.clientId) === normalizedName ||
      normalizeClientName(client.referralCode) === normalizedName
    ));
  };

  const getReferralSelfConflict = (referralValue = formData.referralCustomer, nextFormData = formData) => {
    if (!referralValue) return '';
    const referralClient = getClientByIdentity(referralValue);

    if (isSameClientName(referralValue, nextFormData.client) || isSameClientName(referralClient?.name, nextFormData.client)) {
      return 'Referral customer cannot be the same as the appointment client.';
    }
    if (isSameClientEmail(referralValue, nextFormData.clientEmail) || isSameClientEmail(referralClient?.email, nextFormData.clientEmail)) {
      return 'Referral customer email cannot be the same as the new client email.';
    }
    if (isSameClientPhone(referralValue, nextFormData.clientPhone) || isSameClientPhone(referralClient?.phone, nextFormData.clientPhone)) {
      return 'Referral customer phone cannot be the same as the new client phone.';
    }

    return '';
  };

  const selectedExistingClient = useMemo(() => getClientByName(formData.client), [rawClients, formData.client]);
  const lockClientContactFields = Boolean(selectedExistingClient || user?.role === 'Client');

  const getReferralCodeForCustomer = (name = '') => {
    const referralClient = getClientByName(name);
    return referralClient?.referralCode || referralClient?.clientId || '';
  };

  const selectedExistingClientCode = selectedExistingClient?.clientId || selectedExistingClient?._id || '';

  const handleClientChange = (val: string) => {
    const selectedClient = getClientByName(val);
    const referredByName = selectedClient?.referredBy?.name || '';
    const canUseReferralProfile = referredByName && !isSameClientName(referredByName, val);
    setFormData({
      ...formData,
      client: val,
      clientPhone: selectedClient?.phone || '',
      clientEmail: selectedClient?.email || '',
      referralCustomer: canUseReferralProfile ? referredByName : '',
      referralCode: canUseReferralProfile ? (selectedClient?.referredBy?.referralCode || selectedClient?.referredBy?.clientId || '') : '',
      membershipId: '',
      bookingType: 'Normal',
      service: '',
      serviceId: '',
  ...
  useEffect(() => {
    ...
    if (user?.role === 'Client') {
      setFormData(prev => ({
        ...prev,
        clientPhone: (user as any)?.phone || prev.clientPhone,
    ...
    setFormData(prev => {
      const nextPhone = selectedExistingClient.phone || '';
      const nextEmail = selectedExistingClient.email || '';
      const referredByName = selectedExistingClient.referredBy?.name || '';
      const canUseReferralProfile = referredByName && !isSameClientName(referredByName, selectedExistingClient.name);
      const currentReferralIsSelf = isSameClientName(prev.referralCustomer, prev.client);
      const nextReferralCustomer = canUseReferralProfile ? referredByName : (currentReferralIsSelf ? '' : prev.referralCustomer || '');
      const nextReferralCode = canUseReferralProfile
        ? (selectedExistingClient.referredBy?.referralCode || selectedExistingClient.referredBy?.clientId || '')
        : (currentReferralIsSelf ? '' : prev.referralCode || '');

      if (
        prev.clientPhone === nextPhone &&
        prev.clientEmail === nextEmail &&
        prev.referralCustomer === nextReferralCustomer &&
        prev.referralCode === nextReferralCode
      ) {
        return prev;
      }
    ...
    isModalOpen,
    selectedExistingClient?._id,
    selectedExistingClient?.phone,
    selectedExistingClient?.email,
    selectedExistingClient?.referredBy?.name,
    selectedExistingClient?.referredBy?.referralCode,
    selectedExistingClient?.referredBy?.clientId,
    user?.role,
    user?.email,
    (user as any)?.phone
  ]);
  ...
  const handleOpenModal = (apt: Appointment | null = null) => {
    if (apt) {
      ...
      setFormData({
        ...
        clientPhone: apt.clientId?.phone || apt.clientPhone || '',
        clientEmail: apt.clientId?.email || apt.clientEmail || '',
        referralCustomer: apt.referralCustomer || '',
      ...
      setFormData({
        ...
        clientPhone: user?.role === 'Client' ? ((user as any).phone || '') : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const referralSelfConflict = getReferralSelfConflict();
    if (referralSelfConflict) errors.referralCustomer = referralSelfConflict;
    if (!lockClientContactFields) {
      if (!formData.clientPhone) {
        errors.clientPhone = 'Phone number is required';
      } else {
        const phoneValidation = validatePhoneNumber(formData.clientPhone, settings?.general?.countryIso || 'QA');
        if (!phoneValidation.isValid) {
          errors.clientPhone = phoneValidation.message || 'Invalid phone number';
        }
      }
      if (!formData.clientEmail) errors.clientEmail = true;
    }
    ...
    if (Object.keys(errors).length > 0) {
      ...
      if (errors.referralCustomer) missingFields.push(typeof errors.referralCustomer === 'string' ? errors.referralCustomer : 'Referral customer cannot be same as client');
      if (errors.clientPhone) {
        missingFields.push(typeof errors.clientPhone === 'string' ? errors.clientPhone : 'Phone');
      }
      if (errors.clientEmail) missingFields.push('Email');
    ...
    try {
      ...
      const selectedReferralCustomer = getClientByName(formData.referralCustomer);
      ...
      const payload = {
        ...
        clientPhone: user?.role === 'Client' ? ((user as any).phone || formData.clientPhone) : (formData.clientPhone || selectedClient?.phone || undefined),
        clientEmail: user?.role === 'Client' ? (user.email || formData.clientEmail) : (formData.clientEmail || selectedClient?.email || undefined),
        referralCustomer: !selectedExistingClient ? (formData.referralCustomer || undefined) : undefined,
        referralCode: !selectedExistingClient ? (formData.referralCode || selectedReferralCustomer?.referralCode || selectedReferralCustomer?.clientId || undefined) : undefined,
            ...
            <div className="rounded-[1.5rem] border border-zen-brown/10 bg-white shadow-sm flex flex-col divide-y divide-zen-brown/5">
              <div className="p-6 sm:p-8">
                ...
                <div className="grid gap-5 lg:grid-cols-2">
                  {user?.role === 'Admin' ? (
                    <ZenDropdown
                      ...
                      onChange={val => {
                        ...
                        setFormErrors((prev: any) => ({ ...prev, branch: false }));
                      }}
                    />
                  ) : (
                  ...
                  />
                  <div className={`lg:col-span-2 grid gap-5 ${selectedExistingClient ? 'lg:grid-cols-[minmax(0,1fr)_220px]' : 'lg:grid-cols-1'}`}>
                    <div className="min-w-0">
                      {user?.role === 'Client' ? (
                        <div className="rounded-2xl border border-dashed border-zen-brown/15 bg-zen-cream/20 p-5 flex flex-col justify-center">
                    ...
                    {selectedExistingClient && (
                      <div className="rounded-2xl border border-zen-brown/10 bg-white px-5 py-4 shadow-sm flex flex-col justify-center">
                        <p className="text-[9px] font-black uppercase tracking-[0.3em] text-zen-brown/35">Client ID</p>
                        <p className="mt-1 font-serif text-base font-black text-zen-brown truncate">{selectedExistingClientCode}</p>
                      </div>
                    )}
                  </div>
                  <ZenInput
                    ...
                    error={!lockClientContactFields && formErrors.clientPhone}
                    disabled={lockClientContactFields}
                    placeholder={selectedExistingClient ? 'Phone not saved in profile' : 'Enter phone number'}
                    onChange={(event: any) => {
                      if (lockClientContactFields) return;
                      setFormData({ ...formData, clientPhone: event.target.value });
                      setFormErrors((prev: any) => ({ ...prev, clientPhone: false }));
                    }}
                  />
                  <ZenInput
                    label="Client email"
                    ...
                    error={!lockClientContactFields && formErrors.clientEmail}
                    disabled={lockClientContactFields}
                    placeholder={selectedExistingClient ? 'Email not saved in profile' : 'Enter email address'}
                    onChange={(event: any) => {
                      if (lockClientContactFields) return;
                      const nextFormData = { ...formData, clientEmail: event.target.value };
                      setFormData(nextFormData);
                      setFormErrors((prev: any) => ({ ...prev, clientEmail: false }));
                    }}
                  />
                  {lockClientContactFields && (
                    <div className="lg:col-span-2 rounded-2xl border border-zen-brown/10 bg-zen-cream/25 px-4 py-3 text-[10px] font-bold uppercase tracking-[0.2em] text-zen-brown/35">
                      Contact details are synced from the existing client profile.
                    </div>
                  )}
                  {!selectedExistingClient && (
                    <div className="lg:col-span-2 grid gap-5 lg:grid-cols-2">
                      <ZenAutocomplete
                        ...
                        placeholder="Search referring client by name, email, or ID"
                        options={referralCustomerOptions}
                        subtextKey="email"
                        value={formData.referralCustomer}
                        onChange={(val: any) => {
                          const conflict = getReferralSelfConflict(val);
                          if (conflict) {
                            setFormData({
                              ...formData,
                              referralCustomer: '',
                            ...
                            setFormErrors((prev: any) => ({ ...prev, referralCustomer: conflict }));
                            notify('error', 'Invalid Referral', conflict);
                            return;
                          }
                          const code = getReferralCodeForCustomer(val);
                          setFormData({
                            ...formData,
                            referralCustomer: val,
                            referralCode: code
                          });
                          setFormErrors((prev: any) => ({ ...prev, referralCustomer: false }));
                        }}
                        icon={UserIcon}
                        allowCustom={true}
                      />
                      {formErrors.referralCustomer && (
                        <p className="lg:col-span-2 -mt-3 px-1 text-[10px] font-bold uppercase tracking-[0.18em] text-rose-500">
                          {typeof formErrors.referralCustomer === 'string' ? formErrors.referralCustomer : 'Referral customer cannot be the same as the appointment client.'}
                        </p>
                      )}
                      <ZenInput
                        label="Referral code"
                        ...
                        placeholder="Auto or manual referral code"
                        disabled
                        onChange={() => undefined}
                      />
                    </div>
