        ...
        <div className="flex overflow-x-auto overflow-y-visible pt-2 pb-4 gap-6 lg:grid lg:grid-cols-3 lg:gap-8 lg:overflow-visible scrollbar-hide px-4 lg:px-2">
          {[
            ...
            },
            {
              ...
              bg: 'bg-sky-500/10',
              glow: 'bg-sky-500/20',
              trend: typeof dateRange === 'string'
                ? (dateRange === 'All' ? 'All dates' : `Last ${dateRange.toLowerCase()}`)
                : (dateRange?.from ? `From ${dayjs(dateRange.from).format('DD MMM')} to ${dayjs(dateRange.to || dateRange.from).format('DD MMM')}` : 'Custom Range')
            },
            {
              label: 'Entries',
