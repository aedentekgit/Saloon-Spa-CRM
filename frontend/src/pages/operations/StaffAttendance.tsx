...
const StaffAttendance: React.FC = () => {
  ...
  const { startDate: start, endDate: end } = useMemo(() => {
            ...
            >
              <div className="flex flex-col sm:flex-row items-center justify-between mb-8 gap-4">
                <div className="text-center sm:text-left">
                  <h3 className="text-xl md:text-2xl font-serif font-black text-zen-brown tracking-tight">{periodLabel} Insights</h3>
                  <p className="text-[8px] md:text-[10px] font-bold text-zen-brown/30 uppercase tracking-[0.3em]">Temporal Presence Registry</p>
                </div>
                <ZenMasterCalendar
                  label="Date Range"
                  value={dateRange}
                  onChange={(val: any) => setDateRange(val || 'Month')}
                  selectionType="range"
                  variant="pill"
                  hideLabel
                  className="w-[180px]"
              ...
              <div className="grid grid-cols-7 gap-1 md:gap-4">
                ...
                {(() => {
                  if (!start || !end) return [];
                  const fStart = dayjs(start);
                  const fEnd = dayjs(end);
                  const startDay = fStart.day();
                  const cycleDays = fEnd.diff(fStart, 'day') + 1;
