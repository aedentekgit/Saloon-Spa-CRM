...
const Services = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    commissionValue: 10,
    referralCommissionType: 'Percentage' as 'Percentage' | 'Fixed',
  ...
  const handleOpenModal = (service: Service | null = null) => {
    ...
    if (service) {
      ...
      setFormData({
        ...
        referralCommissionType: primaryBranchEntry?.referralCommissionType || service.referralCommissionType || 'Percentage',
        referralCommissionValue: primaryBranchEntry?.referralCommissionValue ?? service.referralCommissionValue ?? 0,
             ...
             >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 sm:gap-x-16 gap-y-10 sm:gap-y-14 animate-in fade-in duration-500">
                   ...
                   <ZenDropdown
                      ...
                      options={['None', 'Referral Discount', ...serviceCategories.filter(category => category !== 'Referral Discount')]}
                ...
                <div className="md:col-span-2 space-y-8 p-10 bg-zen-sand/5 rounded-[1.5rem] border border-zen-sand/10 shadow-inner">
                   <div className="flex items-center justify-between pb-4 border-b border-zen-brown/10 mb-8">
                      ...
                      <ZenButton
                        type="button"
                        variant="outline"
                        className="text-xs py-2 px-4 shadow-sm border border-zen-brown/15 bg-white text-zen-brown hover:bg-zen-cream/30 hover:border-zen-sand/40 hover:shadow-md transition-all duration-300 active:scale-95 cursor-pointer rounded-xl flex items-center gap-2"
                        onClick={() => {
                          setFormData({
                            ...formData,
                      ...
                      >
                        <Plus size={14} className="text-zen-sand" /> Add Product
                      </ZenButton>
                   </div>
                   ...
                   {formData.inventoryUsage.length === 0 ? (
                     ...
                     <div className="space-y-4">
                       {formData.inventoryUsage.map((usage: any, index: number) => (
                         <div key={index} className="flex flex-col md:flex-row gap-4 items-end bg-gradient-to-r from-white/70 to-white/40 backdrop-blur-sm p-6 rounded-[1.5rem] border border-zen-brown/10 shadow-sm hover:shadow-md hover:border-zen-sand/20 transition-all duration-500 group">
                             ...
                             <button
                               type="button"
                               className="h-11 md:h-12 w-12 flex items-center justify-center rounded-xl border border-rose-100 bg-rose-50/60 text-rose-600 hover:bg-rose-50 hover:text-rose-700 hover:shadow-sm active:scale-95 transition-all duration-300 cursor-pointer flex-shrink-0 mb-0.5"
                               onClick={() => {
           ...
           {activeTab === 'commission' && (
             ...
             >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10 p-10 bg-zen-sand/5 rounded-[1.5rem] border border-zen-sand/10 shadow-inner">
                   <div className="md:col-span-2 pb-4 border-b border-zen-brown/10 flex items-center justify-between">
                      <p className="text-[11px] font-bold text-zen-brown/40 uppercase tracking-[0.4em] flex items-center gap-3">
                         <Zap size={14} className="text-zen-sand" /> Referral Commission Rules
                      </p>
                      <ZenBadge variant="sand" className="scale-90 opacity-40">Referred Clients</ZenBadge>
                   </div>
                   <ZenDropdown
                      label="Referral Commission Type"
                      options={['Percentage', 'Fixed']}
                      value={formData.referralCommissionType}
                      onChange={(val: any) => setFormData({...formData, referralCommissionType: val})}
                   />
                   <ZenInput
                      label={formData.referralCommissionType === 'Percentage' ? "Referral Value (%)" : `Referral Value (${settings?.general?.currencySymbol || 'QR'})`}
                      icon={Sparkles}
                      type="number"
                      placeholder="e.g. 15"
                      value={formData.referralCommissionValue}
                      onChange={(e: any) => setFormData({...formData, referralCommissionValue: parseInt(e.target.value) || 0})}
                   />
                </div>
             </motion.div>
           )}
